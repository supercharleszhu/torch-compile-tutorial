...
Topics covered:
  ...
  - Using a debug backend with tracing enabled
...
# Debug backend
# ---------------------------------------------------------------------------
# The debug backend wraps TorchInductor with extra instrumentation:
...
#      DOT files are generated showing before/after state of each pass.
...
#
# Under the hood, DebugBackend.__call__ applies your custom passes, then
...
def noop_pass(gm: torch.fx.GraphModule) -> None:
    ...
    enabled, the GraphTransformObserver will still generate before/after DOT
    files — useful for verifying the pass infrastructure works without
...
compiled = torch.compile(compute, backend=backend)
